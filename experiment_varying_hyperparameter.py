...
def experiment(dataset, noise_pct, hyperparameter, N_RANDOM_STATE, N_SAMPLES, LABEL_BUDGET, SAMPLE_SIZE):
    ...
    if not os.path.exists('scores/varying_hyperparameter/{}/'.format(hyperparameter)):
        os.makedirs('scores/varying_hyperparameter/{}/'.format(hyperparameter))
    file = open('scores/varying_hyperparameter/{}/{}_{}p_{}_{}.csv'.format(
        hyperparameter,
        dataset_name,
        noise_pct,
        N_RANDOM_STATE,
    ...
    for state in range(N_RANDOM_STATE):
        ...
        for sample in range(N_SAMPLES):
            ...
            if hyperparameter == 'nu':
                models = {
                    'nu=1/2': SLADe(contamination_factor=CONT_FACTOR, random_state=state, nu=1/2),
                    'nu=3/2': SLADe(contamination_factor=CONT_FACTOR, random_state=state, nu=3/2),
                    'nu=5/2': SLADe(contamination_factor=CONT_FACTOR, random_state=state, nu=5/2),
                    'nu=inf': SLADe(contamination_factor=CONT_FACTOR, random_state=state, nu=float('inf'))
                }
            elif hyperparameter == 'q':
                models = {
                    'q=0.5': SLADe(contamination_factor=CONT_FACTOR, random_state=state, q=0.5),
                    'q=1': SLADe(contamination_factor=CONT_FACTOR, random_state=state, q=1),
                    'q=2': SLADe(contamination_factor=CONT_FACTOR, random_state=state, q=2),
                    'q=5': SLADe(contamination_factor=CONT_FACTOR, random_state=state, q=5),
                    'q=10': SLADe(contamination_factor=CONT_FACTOR, random_state=state, q=10)
                }
            elif hyperparameter == 'prior':
                models = {
                    'IForest': SLADe(contamination_factor=CONT_FACTOR, random_state=state, 
                                     unsupervised_model=iforest.IForest(contamination=CONT_FACTOR)),
                    'LOF': SLADe(contamination_factor=CONT_FACTOR, random_state=state, 
                                     unsupervised_model=lof.LOF(contamination=CONT_FACTOR)),
                    'kNNO': SLADe(contamination_factor=CONT_FACTOR, random_state=state, 
                                     unsupervised_model=knn.KNN(contamination=CONT_FACTOR)),
                    'OCSVM': SLADe(contamination_factor=CONT_FACTOR, random_state=state, 
                                     unsupervised_model=ocsvm.OCSVM(contamination=CONT_FACTOR)),
                }
...
if __name__ == "__main__":
    ...
    parser.add_argument('-hyperparameter', required=True)
    ...
    HYPERPARAMETER = str(args.hyperparameter)
                    
    experiment(dataset, 0.1, HYPERPARAMETER, N_RANDOM_STATE, N_SAMPLES, LABEL_BUDGET, SAMPLE_SIZE)
